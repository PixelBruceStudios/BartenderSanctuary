...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (req.method === 'POST') {
    ...
    if (userId) {
      ...
      const testRows: any[] = await query('SELECT scope, lesson_id, passing_score FROM tests WHERE id = $1', [testId]);
      ...
      await query(
        `INSERT INTO test_attempts (test_id, user_id, session_id, score, passed, answers)
         VALUES ($1,$2,$3,$4,$5,$6)`,
        [testId, null, session_id || 'anon', score ?? 0, !!passed, JSON.stringify(answers ?? [])]
      );
    }
